...
class DDQN(object):
    ...
    def learn(self):                                                            # 定义学习函数(记忆库已满后便开始学习)
        ...
        q_target = b_r + GAMMA * q_target_s_a_prime.view(BATCH_SIZE, 1)
...
for i in range(400):                                                    # 400个episode循环
    ...
    while True:                                                         # 开始一个episode (每一个循环代表一步)
        ...
        ddqn.store_transition(s, a, new_r, s_)                 # 存储样本
        episode_reward_sum += new_r                           # 逐步加上一个episode内每个step的reward

        s = s_                                                # 更新状态

        if ddqn.memory_counter > MEMORY_CAPACITY:              # 如果累计的transition数量超过了记忆库的固定容量2000
